...
RandComp::RandComp(Board startBoard, int n, bool b):Player(){
     srand ( (unsigned int)time(NULL) );
     board=startBoard;
     playerNum=n;
	 setVisible(b);
}
RandComp::RandComp():Player(){cerr<<"Inside empty RandComp default constructor"<<endl;}
Point RandComp::placeFirstSettlement(){
...
Point RandComp::placeFirstRoad(Point p){
...
Point RandComp::placeSecondSettlement(){
	return placeFirstSettlement();
}

Point RandComp::placeSecondRoad(Point p){
	return RandComp::placeFirstRoad(p);
}
void RandComp::discard(int turn){
...
vector<int> RandComp::canPlaceCity(){
...
vector<int> RandComp::canPlaceSettlement(){
     vector<int> resourcesToTrade;
...
vector<int> RandComp::canPlaceRoad(){
     vector<int> resourcesToTrade;
     if(roads.size()>=15){
		  resourcesToTrade.clear();
          resourcesToTrade.push_back(-1);
...
int RandComp::placeCity(int playerResources[][5], int playerNumPoints[], int turn){
...
int RandComp::placeSettlement(vector<Point> possibleSettlements, int playerResources[][5], int playerNumPoints[], int turn){
...
int RandComp::placeRoad(vector<DoublePoint> possibleRoads, int playerResources[][5], int playerNumPoints[], int turn){
     ...
     if(resources[3]>0 && resources[4]>0 && possibleRoads.size()>0){
          ...
          addRoad(possibleRoads.at(roadIndex).getP1(),possibleRoads.at(roadIndex).getP2(),playerNum-1);
...
int RandComp::takeTurn(vector<DoublePoint> roadPoints, vector<Point> settlePoints, int playerResources[][5], int playerNumPoints[], int turn){
	newRoadPoints.clear();
